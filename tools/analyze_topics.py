...
def preprocess_text(text):
    ...
    text = re.sub(
        r"[^a-záéíóúüñ\s-]", "", text
    )  # Eliminar puntuación y números, pero permitir guiones
    words = text.split()
...
def analyze_topics(input_dir, num_topics=10, num_top_words=10):
    ...
    tfidf_vectorizer = TfidfVectorizer(
        max_df=0.95, min_df=2, stop_words=stopwords.words("spanish")
    )
    ...
    # Aplicar NMF
    nmf_model = NMF(
        n_components=num_topics, random_state=1
    )  # Removed alpha and l1_ratio
    nmf_model.fit(tfidf)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Analiza temas en documentos de texto."
    )
    parser.add_argument(
        "input_dir", type=str, help="Directorio con los archivos .txt procesados."
    )
    parser.add_argument(
        "--num_topics", type=int, default=10, help="Número de temas a extraer."
    )
    parser.add_argument(
        "--num_top_words",
        type=int,
        default=10,
        help="Número de palabras principales por tema.",
    )
